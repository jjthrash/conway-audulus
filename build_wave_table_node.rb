...
def scale_samples(samples_16_bit)
  samples_16_bit.map {|sample|
    (sample.to_f / 0x7FFF.to_f) - 1.0
  }
end

def build_patch(samples)
  doc = build_init_doc
  patch = doc['patch']
...
if __FILE__ == $0
  ...
  samples =
    File.open(ARGV[0]) do |file|
      scale_samples(load_samples(file))
    end
#  samples = 256.times.map {|i|
#    x = i.to_f/255.0*2*Math::PI
#    Math.sin(x)
#  }
  File.write('wavetable.audulus', JSON.generate(make_subpatch(build_patch(samples)['patch'])))
end
